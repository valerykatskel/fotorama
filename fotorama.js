 ...
 * forked by TUT.BY
 * все изменения в коде помечены
 * 'tutby_added_start'        - начало блока кода, который был добавлен
 * 'tutby_added_end'          - конец блока кода, который был добавлен
 * 'tutby_replaced'           - код, который был изначально
 * 'tutby_replaced_to_start'  - начало блока кода, которым был замещен оригинальный
 * 'tutby_replaced_to_end'    - конец блока кода, которым был замещен оригинальный
 */
...
var $WINDOW = $(window),
    ...
    OPTIONS = {
      ...
      /*tutby_added_start*/
      thumbsfit: 'cover', // 'contain' || 'scaledown' || 'none'
      /*tutby_added_end*/
...
jQuery.Fotorama = function ($fotorama, opts) {
  ...
  function thumbsDraw (pos, loadFLAG) {
    ...
    $navThumbFrame.each(function () {
      var $this = $(this),
          ...
          // tutby_replaced
          /*specialFit = 'cover';*/
          // tutby_replaced_to_start
          specialFit = opts.thumbsfit;
          // tutby_replaced_to_end
        ...
        $frames
            ...
            .each(function () {
              ...
              var $this = $(this),
                  ...
                  /*tutby_replaced_to_start*/
                  //thumbwidth = (opts.thumbmaxwidth < Math.round(o_thumbSide2 * frameData.data.thumbratio)) ? opts.thumbmaxwidth : Math.round(o_thumbSide2 * frameData.data.thumbratio) || o_thumbSide;
                  thumbwidth = (opts.thumbmaxwidth < Math.round(o_thumbSide2 * frameData.data.thumbratio)) ? opts.thumbmaxwidth : (opts.thumbsequalwidth ?  opts.thumbwidth : (Math.round(o_thumbSide2 * frameData.data.thumbratio) || o_thumbSide));
                  /*tutby_replaced_to_end*/
              frameData.l = left;
  ...
  function slideThumbBorder (time) {
    ...
    /*fix for correct thumb border width*/
    var thumbsGaps = parseInt($('.fotorama__nav__frame--thumb').css('paddingRight')) - 2; 
    var thumbsGaps = (parseInt($('.fotorama__nav__frame--thumb').css('paddingRight')) - 2 > 2) ? (parseInt($('.fotorama__nav__frame--thumb').css('paddingRight')) - 2) : 0; 
    var calcPos = thumbsGaps > 5 ? navFrameData.l + navFrameData.eq*thumbsGaps : navFrameData.l; // хардкод фикс для случая, когда нужно иметь разные отступы между тумбочками 2 это отступ по умолчанию, очень захардкожено.
    var thumbWidth = thumbsGaps > 5 ? navFrameData.w : navFrameData.w - opts.thumbborderwidth * 2 ;
    /**/
    slide($thumbBorder, {
      time: time * .9,
      pos: calcPos,
